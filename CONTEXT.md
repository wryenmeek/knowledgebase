...
| sensitive paths | The explicit allowlist of paths whose modification triggers heightened review and notification: `wiki/`, `schema/`, `docs/decisions/`, `AGENTS.md`, `.github/copilot-instructions.md`, `scripts/kb/contracts.py`, `scripts/kb/write_utils.py`, `raw/processed/SPEC.md`, `.pre-commit-config.yaml`, `.github/workflows/`. Referenced by `.github/CODEOWNERS` (advisory notification) and the commit-scope check (gate B). Issue #342. Not to be confused with the CI-3 trusted-trigger-model `sensitive_paths` list in `tests/kb/test_ci3_workflow.py`, which gates manual workflow dispatch with a partially overlapping but distinct path set. |
