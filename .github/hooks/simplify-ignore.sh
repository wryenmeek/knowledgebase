...
EVENT_NAME=$(printf '%s' "$INPUT" | jq -r '.hookEventName // empty' 2>/dev/null || printf '')
WORKSPACE_CWD=$(printf '%s' "$INPUT" | jq -r '.cwd // empty' 2>/dev/null || printf '')
if [ -z "$WORKSPACE_CWD" ]; then
  WORKSPACE_CWD="${CLAUDE_PROJECT_DIR:-.}"
fi
CACHE="$WORKSPACE_CWD/.claude/.simplify-ignore-cache"
...
FILE_PATH=$(printf '%s' "$INPUT" | jq -r '.tool_input.file_path // .tool_input.filePath // (if (.tool_input.files | type) == "array" and (.tool_input.files | length) > 0 then .tool_input.files[0] else empty end) // empty' 2>/dev/null) || {
  parse_error="failed to parse file path from .tool_input"
  FILE_PATH=""
}
...
if [ -n "$FILE_PATH" ] && [ "${FILE_PATH#/}" = "$FILE_PATH" ]; then
  FILE_PATH="$WORKSPACE_CWD/$FILE_PATH"
fi
...
is_read_tool() {
  case "$1" in
    Read|read_file|readFile) return 0 ;;
    *) return 1 ;;
  esac
}
...
is_write_tool() {
  case "$1" in
    Edit|Write|apply_patch|create_file|replace_string_in_file|editFiles)
      return 0
      ;;
    *)
      return 1
...
if [ "$EVENT_NAME" = "Stop" ] || [ -z "$TOOL_NAME" ]; then
  [ -d "$CACHE" ] || exit 0
...
if [ "$EVENT_NAME" = "PreToolUse" ] && is_read_tool "$TOOL_NAME"; then
  [ -f "$FILE_PATH" ] || exit 0
...
if [ "$EVENT_NAME" = "PostToolUse" ] && is_write_tool "$TOOL_NAME"; then
  ID=$(file_id "$FILE_PATH")
  [ -f "$CACHE/${ID}.bak" ] || exit 0
