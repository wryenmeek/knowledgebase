...
| customizations lock | The file `.github/.customizations.lock` — concurrency guard for approved `--apply` mode writes to `.github/**` introduced by ADR-028. Sibling to `wiki/.kb_write.lock` and `raw/.rejection-registry.lock`; never held simultaneously with either once implemented. |
