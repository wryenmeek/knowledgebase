...
EXPECTED_WRITE_SURFACE_MATRIX_ROWS: dict[str, dict[str, tuple[str, ...]]] = {
    ...
    "scripts/fleet/**": {
        ...
        "Writable paths": ("None within the repository", "non-governed fleet artifacts"),
        "Lock requirements": ("None within the repository", "FLEET_MUTATION_MAX_ATTEMPTS"),
        "Artifact / schema owners": ("@google/jules-sdk", "scripts/fleet/_fleet_output.ts", "scripts/fleet/preMergeSanityCheck.ts", "ADR-019", "ADR-032"),
        "Hard-fail behavior": ("missing env", "Jules quota saturation", "soft-warn", "pre-merge sanity check failure", "fail closed"),
    },
    "scripts/fleet/fleet-plan.ts": {
        ...
        "Artifact / schema owners": ("ADR-019", "ADR-033", "@google/jules-sdk"),
        "Hard-fail behavior": ("Jules quota saturation", "soft-warn", "0/0/0 staged-diff", "fail closed"),
    },
    "scripts/fleet/fleet-dispatch.ts": {
        "Runtime mode": ("blocking-only", "external", "issue-label transitions"),
        "Writable paths": (".fleet/<date>/sessions.json", "non-governed fleet artifact"),
        "Lock requirements": ("None within the repository", "fleet-dispatch-label-driven"),
        "Artifact / schema owners": ("ADR-019", "ADR-033", ".github/workflows/fleet-dispatch-after-merge.yml", "@google/jules-sdk"),
        "Hard-fail behavior": ("missing/invalid planning artifact", "base-branch ≠ `main`", "label-event pagination failure", "0/0/0 staged-diff", "fail closed"),
    },
    "scripts/fleet/preMergeSanityCheck.ts": {
        "Runtime mode": ("read-only only", "local gate"),
        "Writable paths": ("None", "writes are forbidden"),
        "Read-only / prerequisite paths": ("git diff --cached --name-status --no-renames -z", "git check-ignore --verbose"),
        "Lock requirements": ("None",),
        "Artifact / schema owners": ("scripts/fleet/preMergeSanityCheck.ts", "ADR-019"),
        "Hard-fail behavior": ("Missing expected paths", "0/0/0 staged diff", "unexpected staged path", "staged deletion", "fail closed"),
    },
    "scripts/fleet/fleet-merge.ts": {
        "Runtime mode": ("blocking-only", "external", "issue-label cleanup"),
        "Writable paths": ("None within the working-tree repo",),
        "Lock requirements": ("None within the repository",),
        "Artifact / schema owners": ("ADR-019", "ADR-033", ".github/workflows/fleet-merge.yml", "scripts/fleet/github/pr-file-sanity.ts", "@google/jules-sdk"),
        "Hard-fail behavior": ("missing sessions.json", "merge conflict", "issue-label mutation failure", "0/0/0 PR diff", "fail closed"),
    },
    ...
    "scripts/fleet/archive-stale-sessions.ts": {
        "Runtime mode": ("Mixed", "read-only only", "blocking-only", "external", "GitHub issue-label"),
        "Writable paths": ("None within the repository",),
        "Lock requirements": ("None within the repository", "`jules-archive-approval` GitHub environment"),
        "Artifact / schema owners": ("@google/jules-sdk", "scripts/fleet/fleet-dispatch.ts", "ADR-033", ".github/workflows/jules-archive-stale.yml"),
        "Hard-fail behavior": ("deny-by-default", "whitespace-only `--source-filter`", "missing session→issue join", "fail closed"),
    },
