...
import re
from tests.kb._workflow_yaml import extract_named_step_block, parse_job_mapping_block, parse_top_level_mapping_block
...
class TestCi2WorkflowContract:
    ...
    def test_permissions_match_tp_analyst_readonly(self) ->None:
        ...
        assert re.search(
            '(?im)^\\s*(actions|checks|contents|pull-requests|issues|packages|id-token)\\s*:\\s*write\\s*$'
            , self.workflow_text) is None, 'Workflow must not request write token scopes'
        job_perms = parse_job_mapping_block(self.workflow_text,
            'analyst-diagnostics', 'permissions', WORKFLOW_PATH)
        assert job_perms == {'actions': 'read', 'checks': 'read',
    ...
    def test_workflow_is_diagnostics_only_with_explicit_failures(self) ->None:
        ...
        assert re.search(
            ...
            , self.workflow_text, flags=re.DOTALL
            ) is not None, 'Closure evidence command must include lookback, issue-limit, and closed-after flags'
        ...
        assert 'exit 1' in self.workflow_text
        forbidden_write_or_release_commands = ('git push', 'git commit',
            'gh pr', 'scripts/kb/update_index.py --write',
    ...
    def test_diagnostics_step_propagates_lint_and_test_failures(self) ->None:
        diagnostics_step = extract_named_step_block(self.workflow_text,
            'Run analyst diagnostics (lint + unit tests)', workflow_path=
            WORKFLOW_PATH)
        ...
        closure_step = extract_named_step_block(self.workflow_text,
            'Check issue closure evidence', workflow_path=WORKFLOW_PATH)
        ...
        assert re.search(
            'python3 \\.github/skills/validate-wiki-governance/logic/validate_wiki_governance\\.py.*?wrapper_exit=\\"\\$\\{PIPESTATUS\\[0\\]\\}\\"'
            , self.workflow_text, flags=re.DOTALL
            ) is not None, 'Wrapper command status must be captured for final diagnostics exit_code'
        assert re.search(
            'python3 scripts/validation/check_doc_freshness\\.py.*?freshness_exit=\\"\\$\\{PIPESTATUS\\[0\\]\\}\\"'
            , self.workflow_text, flags=re.DOTALL
            ) is not None, 'Freshness command status must be captured for final diagnostics exit_code'
        assert re.search(
            'python3 scripts/reporting/content_quality_report\\.py.*?quality_exit=\\"\\$\\{PIPESTATUS\\[0\\]\\}\\"'
            , self.workflow_text, flags=re.DOTALL
            ) is not None, 'Quality report command status must be captured for final diagnostics exit_code'
        assert re.search(
            'python3 -m scripts\\.validation\\.check_issue_closure_evidence.*?closure_evidence_exit=\\"\\$\\{PIPESTATUS\\[0\\]\\}\\"'
            , self.workflow_text, flags=re.DOTALL
            ) is not None, 'Closure evidence command status must be captured in the dedicated closure-evidence step'
        ...
        assert re.search(
            'python3 scripts/kb/lint_wiki\\.py --wiki-root wiki --strict.*?lint_exit=\\"\\$\\{PIPESTATUS\\[0\\]\\}\\"'
            , self.workflow_text, flags=re.DOTALL
            ) is not None, 'Lint command status must be captured for final diagnostics exit_code'
        assert re.search(
            'python3 -m pytest tests/ -q.*?tests_exit=\\"\\$\\{PIPESTATUS\\[0\\]\\}\\"'
            , self.workflow_text, flags=re.DOTALL
            ) is not None, 'Test command status must be captured for final diagnostics exit_code'
    ...
    def test_closure_evidence_token_is_step_scoped(self) ->None:
        ...
        assert self.workflow_text.count('GH_TOKEN:'
            ) == 1, 'CI-2 workflow must bind GH_TOKEN exactly once in the closure-evidence step'
...
class TestWorkflowYamlSyntax:
    ...
    def test_all_workflow_yaml_files_parse_without_error(self) ->None:
        ...
        assert workflows_dir.is_dir(
            ), f'Missing workflows dir: {workflows_dir}'
        ...
        assert errors == [
            ], 'Workflow YAML syntax errors found:\n' + '\n'.join(errors)
