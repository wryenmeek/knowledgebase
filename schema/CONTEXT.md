...
| runtime-budgets | `schema/runtime-budgets.json` — canonical runtime budget thresholds (`target_seconds`, `warn_pct`, `fail_pct`) for CI stage timing enforcement and fail-closed classification. |
...
| `runtime-budgets.json` | Canonical CI runtime budget thresholds (`target_seconds`, `warn_pct`, `fail_pct`) used by workflow runtime-budget evaluators. |
