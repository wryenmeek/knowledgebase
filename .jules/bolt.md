...
## 2024-08-01 - [Avoid O(N) array allocation before map]
**Learning:** Accumulating the results of `rglob` or loops into an intermediate list (e.g. `page_paths = []` followed by `.append()`) before passing it into `executor.map()` forces an unnecessary $O(N)$ memory allocation and prevents streaming.
**Action:** Use a generator expression to feed data lazily into `executor.map()`, which reduces peak memory usage and improves efficiency.
