...
## 2024-05-19 - [Performance] Fast-fail before splitlines() allocation
**Learning:** Using `splitlines()` on text blocks unconditionally tokenizes the entire string into an $O(N)$ memory array. If a parsing operation is searching for a specific key or prefix that might be absent (such as extracting optional keys from frontmatter), evaluating the absence via an `O(N)` list iteration is substantially slower than a native substring search `if key not in text:`.
**Action:** Always include a fast-path literal check (e.g., `if key not in text:`) before calling `splitlines()` to completely avoid the memory array allocation and string tokenization overhead when the target pattern is absent.
