...
## 2026-06-25 - [Performance] Fast-path literal check before splitlines()
...
**Action:** Include a fast-path substring check before using `.splitlines()` on text blocks to quickly rule out absent keys and prevent unnecessary memory allocations and looping.
