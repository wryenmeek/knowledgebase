...
## 2024-05-24 - Efficient string line counting
**Learning:** Using `len(content.splitlines())` allocates an O(N) list, consuming unnecessary memory.
**Action:** Use `(content.count('\n') + (0 if content.endswith('\n') else 1) if content else 0)` to count lines with O(1) memory.
