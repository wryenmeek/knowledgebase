...
## 2024-05-04 - [Python Performance] Regex instead of splitlines for file headers
**Learning:** Using `str.splitlines()` on an entire large text file just to extract a small header (like markdown frontmatter) is a massive performance anti-pattern. It forces the interpreter to tokenize the entire string into memory row-by-row, creating immense latency and memory overhead, especially for large markdown pages generated by the knowledgebase.
**Action:** Replace `splitlines()` loops with a fast-path literal check (e.g., `text.startswith()`) combined with a properly anchored regular expression (e.g., `re.DOTALL | re.MULTILINE`) and slicing to extract headers without tokenizing the entire document body.
