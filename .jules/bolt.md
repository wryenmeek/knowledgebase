...
## 2024-08-01 - [EAFP for Path Extraction]
**Learning:** Using `is_relative_to()` followed by `relative_to()` evaluates path resolution twice on the happy path.
**Action:** Use a `try...except ValueError` block around `relative_to()` when the relative path itself needs to be extracted, avoiding redundant checks.
