...
**Learning:** Legacy test files using `unittest.TestCase` are governed by a test framework ratchet (`MAX_UNITTEST_FILES`) in `scripts/kb/contracts.py`. Any non-docstring edits to these files requires migrating them to `pytest` in the same commit, and decrementing the `MAX_UNITTEST_FILES` value by 1. The migration must cleanly remove `unittest.TestCase` inheritance, replace `setUpClass` with appropriate class-level fixtures or class attributes, properly convert assertions to standard `assert` statements, handle context managers like `assertRaises` cleanly, and gracefully adapt `self.subTest()` blocks into parameterization or flat loops rather than using meaningless wrappers like `if True`.
**Action:** When migrating `unittest` test files, always ensure `subTest` invocations are mapped to clean `for` loops or Pytest parameterization, decrement `MAX_UNITTEST_FILES` in `scripts/kb/contracts.py` and `tests/kb/test_contracts.py` respectively, and ensure changes avoid unnecessary modifications outside the scope of the migration to avoid failing code reviews.
