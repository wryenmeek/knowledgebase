...
## 2024-05-19 - Avoid splitlines for counting lines
**Learning:** Using `len(content.splitlines())` allocates an O(N) list in memory. For just counting lines, `content.count('\n') + ...` is faster and uses O(1) memory.
**Action:** Use the O(1) counting approach instead of `splitlines()` when just getting the line count.
