...
## 2025-02-20 - [Performance] Optimize metadata frontmatter parsing

**Learning:** Using `string.splitlines()` to parse metadata/frontmatter at the beginning of large files is a Python performance anti-pattern, as it causes an O(N) memory allocation for the entire file body.

**Action:** Use a fast-path string check (e.g., `text.startswith('---')`) followed by a pre-compiled regular expression with `re.DOTALL` to extract the top block in near-constant time. Also, use `[ \t]` instead of `\s` for horizontal whitespace to avoid unintentionally consuming newlines.
