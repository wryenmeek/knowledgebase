...
## 2026-09-22 - [Performance] Removing O(N) splitlines() array allocation for YAML lists block extraction
**Learning:** Using `splitlines()` on multiline text blocks (e.g., frontmatter content) to find and extract a multi-line YAML list block unconditionally tokenizes the entire string into an $O(N)$ memory array, creating unnecessary allocations. Finding the key using `re.search` with `re.MULTILINE` and then parsing following list lines via `.find('\n')` completely eliminates the array allocation of lines not related to the list, giving a substantial performance speedup and less memory consumption.
**Action:** When extracting a specific multi-line block (like a YAML list) from a large multiline string, do not use `splitlines()` to iterate through all lines. Use `re.search(pattern, text, re.MULTILINE)` to locate the start, and extract the required subsequent lines using a string slicing loop governed by `.find('\n')`.
## 2024-09-24 - [Performance Anti-Pattern: splitlines() replacement]
**Learning:** Do not replace `splitlines()` with a manual `while` loop using `.find('\n')` in standard iteration logic for parsing multiline strings. While `.find('\n')` loop avoids the $O(N)$ string array allocation, built-in string methods like `splitlines()` use highly optimized C code, whereas manual string slicing loops incur heavy Python bytecode execution overhead. In practice, manual loops are often significantly slower than `splitlines()` and drastically reduce code readability.
**Action:** Do not implement manual string parsing loops (like replacing `splitlines()` with `find('\n')`) just to avoid an array allocation, unless you are extracting a single small targeted block from an extremely large string block. Prioritize code readability over unproven micro-optimizations, and always profile manual string manipulation changes.
