...
## 2026-08-16 - [Performance] Lazy loading validated paths from rglob
**Learning:** Using `list()` or sequential `for` loops on `rglob` results before submitting them to `executor.map` requires an unnecessary sequential filesystem scan. This materializes paths in memory and delays processing for large file trees.
**Action:** When processing a large filesystem tree with `rglob()` for parallel execution, do not populate an intermediate list sequentially. Instead, wrap `rglob` in a generator and yield paths lazily. This allows worker processes to begin dispatching chunks immediately.
