...
## 2026-04-24 - [Frontmatter extraction optimization]
**Learning:** Using `string.splitlines()` to parse metadata or frontmatter at the beginning of large files causes an unnecessary O(N) memory allocation for the entire file body.
**Action:** Use a fast-path string check (e.g., `text.find('\\n')` or `text.startswith('---')`) followed by a pre-compiled regular expression with `re.DOTALL` to extract the top block in near-constant time.
