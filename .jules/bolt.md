...
## 2025-02-18 - Optimized extract_headings to use splitlines()
**Learning:** Do not replace `str.splitlines()` with a manual `while` loop using `text.find('\n')` for standard text parsing. Although the manual loop avoids O(N) memory allocation, it is measurably slower in CPU execution time because `splitlines()` is highly optimized in C and natively supports universal newlines (`\r\n`, `\n`, `\r`).
**Action:** Use `str.splitlines()` for text parsing in Python instead of manually looping through line endings.
