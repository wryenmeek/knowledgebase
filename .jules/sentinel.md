...
## 2026-08-21 - [Fix Git Subprocess Stderr Token Leakage in Git Hooks]
**Vulnerability:** Subprocess `git` commands within `scripts/hooks/check_approval_flag.py`, `scripts/hooks/check_locality_ratchet.py`, and `scripts/hooks/check_test_framework.py` returned raw `stderr` which could leak sensitive GitHub tokens (e.g., `ghp_`, `github_pat_`, etc.) if printed via diagnostic error messages or exception strings.
**Learning:** `subprocess.run` executions do not automatically scrub standard error strings of credentials. Relying on raw stderr from CLI tools like `git` for logs or debug output creates an inadvertent vector for token leakage when errors occur.
**Prevention:** In the wryenmeek/knowledgebase repository, to prevent credential leakage (such as GitHub tokens), always wrap `stderr` generated by external CLI tools (e.g., `subprocess.run` executions for `git` or `gh`) with `scripts._redaction.redact_stderr` before logging, printing, or returning it for exception inclusion. Ensure `sys.path.insert(0, str(Path(__file__).resolve().parents[2]))` is used if `__package__` is missing to correctly import the redaction module.
