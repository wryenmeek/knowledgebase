...
## 2026-07-13 - [Hardcoded Bind Address]
**Vulnerability:** WSGI entrypoints defaulted their bind address (`--host`) to `0.0.0.0`, potentially exposing the server to all network interfaces unintentionally.
**Learning:** Hardcoding `0.0.0.0` as the default bind address poses a network exposure risk (Bandit B104) if the server runs in an untrusted or non-containerized environment.
**Prevention:** Use an environment variable with a safe local fallback like `os.environ.get("HOST", "127.0.0.1")` to ensure security while maintaining flexibility.
