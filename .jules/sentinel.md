...
## 2026-07-12 - [Hardcoded Bind Interface (Bandit B104)]
**Vulnerability:** WSGI server was defaulting to bind to `0.0.0.0`, exposing the service on all network interfaces.
**Learning:** Hardcoding `0.0.0.0` as the default host for web servers or webhook receivers can inadvertently expose services to unintended networks, especially during local development or outside of properly configured container environments.
**Prevention:** Always use an environment variable with a safe local fallback like `os.environ.get("HOST", "127.0.0.1")` to ensure security by default while maintaining configurability for deployment environments.
