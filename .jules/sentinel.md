...
## 2024-07-23 - Prevent Unintended Network Exposure in WSGI Entrypoints
**Vulnerability:** WSGI entrypoints and webhook receivers default to binding on `0.0.0.0`.
**Learning:** Defaulting to `0.0.0.0` exposes the server to all network interfaces, which can lead to unintended exposure in containerized or cloud environments, violating the principle of least privilege.
**Prevention:** Always use an environment variable with a safe local fallback like `os.environ.get('HOST', '127.0.0.1')` to ensure security while maintaining compatibility.
