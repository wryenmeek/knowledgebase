...
## 2026-08-12 - [Unintended Network Exposure via 0.0.0.0]
**Vulnerability:** WSGI entrypoints (relay_http.py) defaulted bind `--host` to `0.0.0.0`, potentially exposing the service to unintended network interfaces (Bandit B104).
**Learning:** Hardcoding `0.0.0.0` as a default fallback can expose development or internal tools to external networks without explicit configuration. Empty environment variable strings (`HOST=""`) also need to be handled to avoid binding to an empty string.
**Prevention:** Always use an environment variable with a safe local fallback (e.g., `os.environ.get("HOST", "").strip() or "127.0.0.1"`) instead of defaulting to `0.0.0.0`.
