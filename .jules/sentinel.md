...
## 2025-01-24 - WSGI Server Network Exposure (Bandit B104)
**Vulnerability:** Hardcoded 0.0.0.0 bind addresses in WSGI entrypoints exposed webhook receivers to all network interfaces.
**Learning:** Defaulting to 0.0.0.0 prioritizes container convenience over local security.
**Prevention:** Use an environment variable with a safe local fallback (`os.environ.get('HOST', '127.0.0.1')`).
