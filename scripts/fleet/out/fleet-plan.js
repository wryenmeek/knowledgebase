...
`);
    ...
    for (const line of lines) {
      if (line.startsWith("--- ")) {
        ...
        toPath = line.slice(4).replace(/^b\//, "").replace(/^\/dev\/null$/, "");
      }
    }
    let changeType;
    ...
    files.push({ path: path2, changeType, additions, deletions });
  }
  return files;
}
...
`);
    files.push({ path: path2, changeType, content, additions, deletions });
  }
  return files;
}
...
function parseSelectExpression(expr) {
  ...
  const path2 = cleanPath.split(".").filter((p) => p.length > 0);
  return { path: path2, exclude, wildcard: false };
}
function getPath(obj, path2) {
  ...
  if (Array.isArray(obj)) {
    const results = obj.map((item) => getPath(item, path2)).filter((v) => v !== undefined);
...
function setPath(obj, path2, value) {
  if (path2.length === 0 || value === undefined)
    return;
  const [head, ...tail] = path2;
  if (tail.length === 0) {
    obj[head] = value;
...
function deletePath(obj, path2) {
  if (path2.length === 0 || obj === null || obj === undefined)
    return;
  if (Array.isArray(obj)) {
...
class NodeFileStorage {
  ...
  async init() {
    if (this.initialized)
      return;
    await fs.mkdir(path$1.dirname(this.filePath), { recursive: true });
    try {
...
class NodeSessionStorage {
  ...
  async upsert(session) {
    ...
    await fs.writeFile(path$1.join(sessionDir, "session.json"), JSON.stringify(cached, null, 2), "utf8");
    const indexEntry = {
      id: session.id,
      title: session.title,
      state: session.state,
    ...
    await fs.appendFile(this.indexFilePath, JSON.stringify(indexEntry) + `
`, "utf8");
  }
...
// prompts/analyze-issues.ts
function analyzeIssuesPrompt({
  issuesMarkdown,
  repoFullName
}) {
  const now = new Date;
...
async function verify(secret, eventPayload, signature) {
  ...
  return timingSafeEqual2(signatureBuffer, verificationBuffer);
}
...
// github/issues.ts
var CachedOctokit = Octokit2.plugin(cachePlugin);
...
async function toIssueDocMarkdown(issues) {
  const repoInfo = await getGitRepoInfo();
  const lines = [
...
async function getIssuesAsMarkdown() {
...
var issuesMarkdown = await getIssuesAsMarkdown();
var prompt = analyzeIssuesPrompt({ issuesMarkdown, repoFullName: repoInfo.fullName });
console.log(`\uD83D\uDD0D Planning fleet for ${repoInfo.fullName} (branch: ${baseBranch})`);
var run = await jules.run({
  prompt,
  source: {
...
if (process.env.GITHUB_OUTPUT) {
  fs2.appendFileSync(process.env.GITHUB_OUTPUT, `plan_session_id=${run.id}
`);
}
