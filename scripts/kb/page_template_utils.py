...
def extract_headings(body: str) -> set[str]:
    ...
    Streams through ``body`` using ``splitlines()`` to leverage C-level optimizations
    for faster text processing compared to manual ``find('\n')`` loops. Skips
    ...
    # ⚡ Bolt: Replace manual find('\n') loop with splitlines() for faster CPU execution
