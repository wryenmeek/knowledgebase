...
def extract_headings(body: str) -> set[str]:
    ...
    Uses ``splitlines()`` to iterate over lines, as it is implemented in C and
    executes significantly faster than a manual Python ``while`` loop using
    ``.find('\n')``, despite the O(N) array allocation. Natively supports
    universal newlines. Skips headings inside fenced code blocks (``` or ~~~).
    """
