...
| `tp-pr-producer` | CI-3 workflow (PR-producing) | `contents: write`, `pull-requests: write`, `actions: read`, `checks: read`, `models: read` | settings/admin/secrets scopes |
...
| CI-3 | Workflow (PR-producing) → repo | trusted handoff for `raw/inbox/**` or explicit maintainer-approved manual trigger | generate/update PR | `tp-pr-producer` | `kb-write-${{ github.repository }}-${{ github.ref }}` + local lock `wiki/.kb_write.lock` | preflight prerequisites = `PASS`; ingest/index/lint return zero; query-persist returns `written` or `no_write_policy`; synthesis stage runs after ingest (soft-fails on LLM errors: extraction writes empty bundle with `soft_skipped: true`, downstream synthesize scripts no-op and exit 0) and before `update_index`; required branch checks and approvals pass before merge | fail closed (non-zero); abort commit/PR update; block merge; reject out-of-allowlist writes |
