...
| Controlled automation | `docs/architecture.md` CI-1 through CI-6 model; declared write-surface matrix in `AGENTS.md` | Static-verified | Strong design and wiring evidence. Actual workflow execution and credential availability are unassessed. |
| Governed agent control plane | `docs/architecture.md` operator lane sequencing and skill-layer inventory | Static-verified | Strong boundary discipline. The framework directs work through policy gates and preserves the Python runtime as the execution authority. |
