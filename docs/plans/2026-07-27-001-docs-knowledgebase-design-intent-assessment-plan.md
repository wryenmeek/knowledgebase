...
- R4. Distinguish the reusable framework from the template's bundled corpus so readers do not infer content maturity from tooling maturity.
...
- **Treat the framework and corpus as separate layers:** The template framework and an instance's domain corpus have separate maturity levels. Strong provenance tooling does not establish that a current corpus is mature, so the assessment must report both layers independently.
...
- Implementing checkpoint mutation wiring, workspace-audit composition, semantic endpoint hosting, provider fallback, test migration, or domain-source ingestion.
...
- **Approach:** Classify confirmed gaps by the R3 taxonomy and impact. Treat the checkpoint registry as implemented with deferred CI-3 per-batch mutation integration. Cover the partially wired workspace-audit orchestrator, the browser-side-but-operator-owned semantic endpoint contract, the expired fleet fallback deferral, the test-framework migration backlog, and the lack of substantive persisted domain analysis separately from framework capability.
...
- `TEMPLATE.md` establishes the reusable framework/content-layer split. `mkdocs.yml` and `.github/skills/references/manual-of-style.md` must remain domain-neutral template defaults.
