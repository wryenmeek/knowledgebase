...
- `kb-source-monitor` exists to ingest external source repositories for the wiki
  pipeline (CI-5). Its scope is intentionally narrow read-only — `contents: read`
  semantics on installations of designated source repos.
- Widening it to `pull_requests: write` + `contents: write` on the *operator's
  own* repos would conflate two unrelated trust domains (source ingestion vs.
  fleet automation) and violate the principle of least privilege.
...
operator setup cost without any blast-radius benefit (one operator, identical
trust boundary).

Three patterns were considered for the new fleet identity:

- **A — Widen `kb-source-monitor`**: rejected (above).
- **B — One App per fleet-running repo**: rejected (4× setup cost, identical
  trust boundary, no blast-radius gain).
