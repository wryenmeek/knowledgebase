...
| 5a | Phase 6 | [#199](https://github.com/wryenmeek/knowledgebase/issues/199) | ✅ CLOSED | `scripts/hooks/check_locality_ratchet.py` basic paired-deletion-or-trailer gate, `.github/hooks/hooks.json` PreCommit registration, and matrix row ([PR #265](https://github.com/wryenmeek/knowledgebase/pull/265)) |
| 5b | Phase 6 (soft budget) | [#200](https://github.com/wryenmeek/knowledgebase/issues/200) | ✅ CLOSED | `scripts/hooks/check_locality_ratchet.py` commit-msg-stage trailer soft-budget mode, `.github/hooks/hooks.json` CommitMsg registration, tests |
...
- [ ] **Locality 3d gate — split across `pre-commit` and `commit-msg` stages** on any commit touching `.github/copilot-instructions.md` **or** `AGENTS.md` — **blocking on both files** per Decision Q3 (multi-consumer reality for `AGENTS.md`). The split is required because git's `pre-commit` stage runs *before* the commit message exists and only receives staged file paths, so the `Locality-4-Justification:` trailer is not visible at `pre-commit` time. `commit-msg` is the only stage that receives the finalized commit-message file path:
    ...
    - **Stage 1 — `pre-commit` hook (`scripts/hooks/check_locality_ratchet.py`).** Read-only; receives staged file paths. Compares the staged diff against HEAD inside the non-exempt regions of each gated file and computes `net_section_delta = additions − deletions` *within the non-exempt sections only* (NOT additions-only). Passes when `net_section_delta ≤ 0`, so a net-negative replacement (e.g., a 3-line stale rule replaced by a 1-line rule: +1/−3, delta = −2) passes even though additions > 0.
        - Actual trailer-aware mechanism: the same script can receive commit-message text through argv/stdin or a hook payload. When `net_section_delta > 0`, it passes only when a `Locality-4-Justification:` trailer is present and, in CommitMsg wiring, within the rolling soft budget.
    - **Stage 2 — `commit-msg` hook (`scripts/hooks/check_locality_ratchet.py` commit-msg mode).** Read-only; receives the prospective commit-message text from the hook wrapper. Reads the message, looks for a `Locality-4-Justification: <reason>` trailer (RFC-5322 trailer format, parsed via `git interpret-trailers --parse`), and enforces the soft budget below. When the staged diff has a positive `net_section_delta`, the commit-msg stage passes iff the trailer is present **and** within budget. When the staged diff already passed on `net_section_delta ≤ 0`, the commit-msg stage is a no-op for that commit.
    - **Wiring.** Register `check_locality_ratchet.py` for both pre-commit and commit-msg hook events so the same read-only implementation checks staged deltas and finalized trailers at the correct boundaries.
    ...
    - **Trailer soft budget (Decision Q10):** the commit-msg hook walks the last 10 commits touching each file's gated region and counts `Locality-4-Justification:` trailers since the latest same-file paired-deletion commit. If 1 trailer is already present in that window, the commit-msg gate hard-fails until a paired-deletion commit lands. Numerically tunable; document the default ratio (1-in-10) in ADR-028. Budget is **per-file** (separate counts for `copilot-instructions.md` and `AGENTS.md`) so a noisy week on one file doesn't starve the other. Contributors should paste the trailer text from `docs/templates/locality-4-justification-trailer.md` (shipped in slice 2 / #192) to ensure the format matches what `git interpret-trailers --parse` will accept.
    ...
    - Implementation: `scripts/hooks/check_locality_ratchet.py` (pre-commit and commit-msg modes; read-only matrix row; `net_section_delta` computation on either file's non-exempt regions; trailer parse via `git interpret-trailers --parse`; soft-budget check via `git log`).
...
- [ ] **Cascade obligations:**
    - Write-surface matrix row updates for `check_locality_ratchet.py`; the PostToolUse advisory row for `scripts/hooks/locality_postuse_advisory.py` landed with #195.
    - `.github/hooks/hooks.json` registers `check_locality_ratchet.py --commit-msg-file` for the CommitMsg hook event so the commit-msg stage actually fires in the Copilot hook surface.
...
- Pre-commit hook precedent in this repo: `scripts/hooks/check_adr_cross_ref.py`, `check_stub_archive_path.py`, `check_context_md_format.py` — `check_locality_ratchet.py` joined this family at the `pre-commit` stage in #199 and added commit-msg-stage trailer soft-budget enforcement in #200.
...
| **trailer soft budget** | The rolling-window cap on `Locality-4-Justification:` git trailers (default 1 per 10 commits to global rules sections) that prevents the escape hatch from normalizing into bypass. Enforcement belongs to `scripts/hooks/check_locality_ratchet.py` at the `commit-msg` stage (paired with the same script's pre-commit line-delta check) after #199/#200. |
