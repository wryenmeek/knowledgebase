...
# deterministic wiki maintenance surfaces (authoritative; all phases complete as of 2026-05-14)
...
# Phase 3 — batch query persistence (scripts/kb/batch_persist_query.py)
python3 scripts/kb/batch_persist_query.py --batch-file <batch.json> --wiki-root wiki --schema AGENTS.md

# Phase 4 — wiki coverage analytics (scripts/reporting/coverage_report.py)
# summary mode (read-only):
python3 scripts/reporting/coverage_report.py --mode summary
# persist mode (requires explicit approval; writes wiki/reports/coverage-report-*.json):
