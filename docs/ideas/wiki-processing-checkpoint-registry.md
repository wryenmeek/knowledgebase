...
- [x] `docs/architecture.md` updated to document checkpoint lifecycle and lock ordering (runtime landed in PR3; CI wiring/bootstrap remains PR4).
- [x] `docs/mvp-runbook.md` updated with: manual rescan procedure, checkpoint recovery procedure (per the rollback-scenarios decision), required bootstrap dry-run-then-apply sequence, and `gh run list --workflow=ci-3-pr-producer.yml --status in_progress` reference for the lock-unavailable case (runtime commands landed in PR3; initial bootstrap execution remains PR4).
