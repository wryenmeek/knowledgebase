...
**Status:** Implemented (Phase 4) — PR4 of 4 (`checkpoint/04-ci-wiring-and-bootstrap`) seeded the initial registry and surfaced `--verify --warn-only` to the CI-3 job summary; CI-3 `--mutate` per-batch wiring is deferred to issue [#377](https://github.com/wryenmeek/knowledgebase/issues/377) (runtime requires a fully-formed mutation input JSON; wrapper deserves its own design pass). Design decisions resolved via grilling session 2026-06-07 and simplified via `improve-codebase-architecture` review on 2026-06-07 (Path C-prime, 4 PRs).
...
4. **PR4 — `checkpoint/04-ci-wiring-and-bootstrap`.** Wire CI-3 to invoke `checkpoint_registry.py --mutate` per batch, followed by `--verify --warn-only` whose output is surfaced to the GitHub Actions job summary; warning-log persistence, if enabled for CI telemetry, must use `--verify --warn-only --log-warnings --approval approved`. Execute the bootstrap runbook sequence (dry-run bootstrap -> review reconciliation report -> operator confirmation -> `--bootstrap --apply --approval approved`) and commit the initial registry JSON. **Scope reduced at implementation:** the `--verify --warn-only` job-summary surface and the HITL bootstrap landed in PR #376; CI-3 `--mutate` per-batch wiring is deferred to issue [#377](https://github.com/wryenmeek/knowledgebase/issues/377) because the runtime contract requires a fully-formed mutation input JSON and no helper exists to construct it from a CI-3 run state.
...
- [x] `docs/architecture.md` updated to document checkpoint lifecycle and lock ordering (runtime landed in PR3; HITL bootstrap + CI verify wiring landed in PR4 / #376).
- [x] `docs/mvp-runbook.md` updated with: manual rescan procedure, checkpoint recovery procedure (per the rollback-scenarios decision), required bootstrap dry-run-then-apply sequence, and `gh run list --workflow=ci-3-pr-producer.yml --status in_progress` reference for the lock-unavailable case (runtime commands landed in PR3; initial bootstrap execution + lock-unavailable diagnostic flip landed in PR4 / #376).
...
- [x] `scripts/kb/CONTEXT.md` `last_updated` bumped when `checkpoint_registry.py` lands (PR3).
- [x] 1 new row added to `AGENTS.md` write-surface matrix: `scripts/kb/checkpoint_registry.py` (single entrypoint with all three modes documented in the row) (PR3).
- [x] `sync-knowledgebase-state` matrix row's Schema/artifact owners column extended to cite the new contract (PR3).
- [x] `docs/ideas/wiki-curation-agent-framework.md` `FRAMEWORK_BOUNDARY_DOCS` entrypoint list extended verbatim with `scripts/kb/checkpoint_registry.py` — required by `tests/kb/test_framework_contracts.py::test_boundary_docs_list_same_execution_surface` (literal `assertIn`).
...
| Pre-commit hook for registry JSON | Invalid JSON discovered at runtime | `--verify --log-warnings --approval approved` mode JSON-parse failure events appended to `wiki/log.md` under event type `checkpoint_registry.verify_warning` | ≥1 corruption incident logged |
| Pre-commit hook for registry JSON | Schema validation failures | `--verify --log-warnings --approval approved` mode schema-check failure events appended to `wiki/log.md` under event type `checkpoint_registry.verify_warning` | ≥2 schema failures from hand-edits in 90 days |
...
| CI-3 `--mutate` per-batch wiring (deferred PR4 → issue [#377](https://github.com/wryenmeek/knowledgebase/issues/377)) | Drift between operator `--mutate` cadence and CI-3 batch state; verify warnings about stale items | `--verify --warn-only` job-summary `summary.warnings[]` plus `gh run list --workflow=ci-3-pr-producer.yml` cadence | ≥3 verify warnings in 30 days, or any item stuck in `in_progress` > 24 h |
