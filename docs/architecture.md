...
| `scripts/hooks/**` | Pre-commit, PostToolUse advisory, and CI governance hook scripts | Hooks are read-only — no repository writes. They enforce local governance guardrails (ADR-016 authorizes this family; ADR-028 adds the PostToolUse locality advisory). |
...
> **Runtime landed; CI wiring in progress.** The schema contract,
...
> `analysis_fingerprint()` helper landed in PR #213. The runtime
> (`scripts/kb/checkpoint_registry.py`) provides the bootstrap/mutate/verify
> entrypoint. CI-3 wiring and the HITL bootstrap execution remain in progress.
...
- [`ADR-028`](decisions/ADR-028-instruction-locality-ladder.md): instruction locality ladder — Locality 0–4 hierarchy and paired-deletion rule for always-on files
