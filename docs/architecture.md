...
| Advisory freshness sweep | `.github/workflows/wiki-freshness.yml` (scheduled Monday 03:30 UTC; `workflow_dispatch` with `enforcement_mode` input) | Runs the stale check, stale annotations, stale routing, and the advisory governance signal sweep, then runs a separate `check_doc_freshness.py --near-expiry-days 14` signal and annotates `near_expiry` pages before uploading the reports. Near-expiry validation never changes the blocking stale gate; the workflow is always advisory unless `blocking` mode is selected. |
...
| `freshness_near_expiry_days` | 14 | `check_doc_freshness.py --near-expiry-days`, `wiki-freshness.yml` | Pages within 14 days of the 90-day freshness threshold are flagged with `near_expiry` for proactive review; already-stale pages retain `stale_document`. |
