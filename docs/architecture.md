...
| `scripts/hooks/**` | Pre-commit, PostToolUse advisory, and CI governance hook scripts | Hooks are read-only — no repository writes. They enforce local governance guardrails (ADR-016 authorizes this family). |
...
> `analysis_fingerprint()` helper landed in PR #213. Runtime landed in PR
> wryenmeek/knowledgebase#233 (`scripts/kb/checkpoint_registry.py`
> bootstrap/mutate/verify entrypoint). CI-3 wiring and HITL bootstrap
...
- [`ADR-028`](decisions/ADR-028-instruction-locality-ladder.md): instruction locality ladder, Locality-4 deletion pairing/trailer governance, and customization-lock declaration
