...
| `jules-archive-stale.yml` | `workflow_dispatch` only | Bulk-archive stale Jules sessions. Inputs: `state` (default `inProgress`), `older_than_days` (required), `source_filter` (optional), `apply` (default `false`/dry-run). Dry-run by default; set `apply=true` to archive. See `.github/instructions/fleet-operations.instructions.md` for runbook. | `cd scripts/fleet && bun run archive-stale-sessions.ts --older-than-days N [--apply]` |
