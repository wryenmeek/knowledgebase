...
| **CI-2** (`.github/workflows/ci-2-analyst-diagnostics.yml`) | read-only diagnostics (`lint_wiki --strict` + test suite); triggers on `push` to `main` and `workflow_dispatch` | run the same diagnostics locally (`lint_wiki`, `pytest tests/`), attach findings to PR/issue; no repo-write automation needed. |
