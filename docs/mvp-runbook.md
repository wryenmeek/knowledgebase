...
- **`GITHUB_TOKEN`-authored merge suppresses Phase 2b** (Layer 6, Issue #310): when Phase 2a (`fleet-dispatch.yml`) queues `gh pr merge --auto` using `GITHUB_TOKEN`, the resulting squash-merge push to `main` does **not** fire any push-triggered workflow (documented GitHub Actions behavior). Symptom: planning PR shows merged by `app/github-actions`, no Phase 2b run, no per-task agent sessions appear, `gh api repos/<owner>/<repo>/commits/<merge_sha>/check-runs` returns empty. Recovery: `gh workflow run fleet-dispatch-after-merge.yml --ref main` — the operator escape hatch re-resolves the planning artifact via fleet-state's `pending_date` and runs the same detect/clear/dispatch path as a normal push trigger. Permanent fix in Issue #310 (switch Phase 2a to a GitHub App installation token so pushes aren't treated as `GITHUB_TOKEN` events).
