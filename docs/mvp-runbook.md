...
| Post-merge body-vs-diff audit | `.github/workflows/post-merge-body-audit.yml` on merged `pull_request: closed` events | Advisory Option A audit for issue #329; runs `scripts/maintenance/audit_pr_body_vs_diff.py --pr <number>` with read-only checkout and PR-comment permission to flag file-path evidence of Type B drift, Type C body hallucinations, and the severity-1 `0 added, 0 deleted, 0 files` signature without blocking merges. |
...
| **CI-2** (`.github/workflows/ci-2-analyst-diagnostics.yml`) | read-only diagnostics (`validate_wiki_governance`, `check_doc_freshness`, `content_quality_report`, `check_issue_closure_evidence` with forward-only cutover, `lint_wiki --strict`, dependency audit, secret scan, and test suite); triggers on `pull_request`, `push` to `main`, and `workflow_dispatch`. On `pull_request`, `check_doc_freshness` is diff-scoped (issue #558): a "Compute freshness check scope" step diffs `wiki/concepts`, `wiki/entities`, and `wiki/analyses` markdown files added/modified against the PR base SHA and passes only those paths to the freshness check (or skips it entirely when the PR touches none of them); `push`-to-`main` and `workflow_dispatch` retain the full repo-wide scan as the systemic staleness safety net. | run the same diagnostics locally (`python3 .github/skills/validate-wiki-governance/logic/validate_wiki_governance.py --quiet`, `python3 -m scripts.validation.check_doc_freshness --scope wiki --path wiki/concepts --path wiki/entities --path wiki/analyses --as-of "$(date -u +%Y-%m-%d)" --max-age-days 90 --failures-only` for the full-scope form, or substitute `--path <changed-file>` per PR-changed file to reproduce the diff-scoped `pull_request` behavior, `python3 scripts/reporting/content_quality_report.py --mode summary --path wiki --failures-only`, `python3 -m scripts.validation.check_issue_closure_evidence --lookback-days 3650 --issue-limit 500 --closed-after 2026-05-25T00:00:00Z`, `python3 scripts/kb/lint_wiki.py --wiki-root wiki --strict`, `python -m pip install --quiet 'pip>=26.1' pip-audit && pip-audit --desc on --ignore-vuln CVE-2026-3219`, `gitleaks detect --source . --config .gitleaks.toml --redact --no-banner`, `python3 -m pytest tests/ -q --cov=scripts/kb --cov=scripts.validation._runtime_budget --cov-fail-under=90`), attach findings to PR/issue; workflow-level permissions stay `actions/checks/contents: read` with `issues: read` scoped to the `analyst-diagnostics` job; no repo-write automation needed. |
...
|---|---|---|---|
| `commit-scope-check.yml` | `pull_request: opened, synchronize, edited` targeting `main` | Gate B (sensitive-paths acknowledgement): blocks when a PR touches a sensitive path (`wiki/`, `schema/`, `docs/decisions/`, `AGENTS.md`, etc.) without naming the surface in the PR title or body first line via word-boundary token match. Gate C (deletion-ratio guard): blocks when net deletions exceed 50 lines without a verified `Reverts:` or `Cleanup:` Git footer trailer. Both gates run independently. Not yet a required check — promotion is pending the 90-day false-positive audit. | `python -m scripts.validation.check_commit_scope` (with `PR_NUMBER`, `PR_TITLE`, `PR_BODY` env vars set) |
...
| `fleet-dispatch.yml` | `pull_request` opened/reopened | Fleet phase 2a: detects Jules planning PRs via `fleet-state` pending session, verifies the PR diff is not 0/0/0 and contains only `.fleet/<pending_date>/issue_tasks.{md,json}`, then queues an `--auto` squash-merge that waits for branch-protection required checks (notably CI-2 diagnostics) to pass. When `FLEET_APP_ID` and `FLEET_APP_PRIVATE_KEY` repository secrets are present and valid, the merge uses the `fleet-orchestrator` App installation token so the resulting push can trigger Phase 2b; otherwise it falls back to `GITHUB_TOKEN` with a warning annotation and Layer 6 suppression still applies. Exits after queuing — the per-task agent dispatch happens in `fleet-dispatch-after-merge.yml` once the merge lands or via the manual escape hatch. | `gh pr merge <N> --auto --squash --delete-branch` |
| `fleet-dispatch-after-merge.yml` | `push` to main (path-filtered to `.fleet/*/issue_tasks.json`) or `workflow_dispatch` (operator escape hatch — see Issue #310 Layer 6) | Fleet phase 2b: fires when a planning artifact lands on main (typically from phase 2a's queued auto-merge). Detects the newly-added artifact, cross-checks against fleet-state's pending session, clears `.fleet/.pending_session`, and runs `fleet-dispatch.ts` to spawn one Jules agent session per task. Fail-closed preflight enforces `FLEET_PENDING_DATE` format (`YYYY_MM_DD`) and rejects path-escaping values. When triggered via `workflow_dispatch`, detection skips the HEAD~1 diff (intervening commits may have landed) and resolves the artifact via fleet-state's pending_date; keep this escape hatch while the optional Phase 2a GitHub App token path is only partially complete or if the App token fallback warning appears. Split from fleet-dispatch.yml on 2026-06-20 to eliminate the race between the merge step and branch-protection required checks. | `gh workflow run fleet-dispatch-after-merge.yml --ref main` (operator) or `cd scripts/fleet && bun run fleet-dispatch.ts` (local) |
| `fleet-merge.yml` | `workflow_run` (CI-2 completes), `workflow_dispatch` | Event-driven sequential merge of Jules-authored PRs: fires when CI-2 passes on a fleet PR head SHA → update branch → verify the PR file list is non-empty → squash merge; re-dispatches on conflict. Manual `workflow_dispatch` sweeps all open fleet PRs with currently passing CI. Fail-closed preflight enforces `FLEET_PENDING_DATE` format (`YYYY_MM_DD`) and merge fails closed when zero check runs exist unless `FLEET_ALLOW_NO_CHECKS=true`. | `cd scripts/fleet && bun run fleet-merge.ts` |
| `fleet-submit-prs.yml` | Daily schedule (cron `0 4 * * *`, 04:00 UTC), `workflow_dispatch` | Sweeps Jules sessions in `awaitingUserFeedback` state for this repository and sends the canonical PR-submission prompt to each one via `jules.session(id).send()`. Also applies the `awaiting-feedback` GitHub label to linked issues discovered via the session's working branch name. Quota saturation soft-warns and exits 0 per ADR-032. `JULES_API_KEY` is step-scoped per the step-scoped secret binding rule. | `cd scripts/fleet && bun run fleet-submit-prs.ts` |
| `jules-account-probe.yml` | Scheduled cron `0 0,9,18 * * *` (3 slots/day at 00:00/09:00/18:00 UTC) + `workflow_dispatch` | Jules account health diagnostic: lists registered sources, active/inProgress session counts per source, and account totals. Attaches JSON artifact. On probe failure posts a sanitized failure envelope (4 allowlisted fields only — no raw error messages or foreign session IDs) to the pinned tracking issue labelled `probe-tracking`; on recovery after sustained failure posts a recovery comment with failure count and first-failure timestamp. Fails closed if the tracking issue is missing or closed. Use to diagnose session-cap saturation (see #82). | `cd scripts/fleet && bun run jules-account-probe.ts` |
| `jules-archive-stale.yml` | `workflow_dispatch` only | Bulk-archive stale Jules sessions. Inputs: `state` (default `inProgress`), `older_than_days` (required), `source_filter` (default `sources/github/wryenmeek/knowledgebase`; required when `apply=true` — script exits non-zero if apply=true and empty; surrounding whitespace trimmed, whitespace-only rejected per PR #315), `apply` (default `false`/dry-run). Dry-run by default; set `apply=true` to archive. **Apply runs (`apply=true`) gate on the `jules-archive-approval` GitHub environment** (configure reviewers in Settings → Environments); dry-runs bypass approval. Concurrency partitions by `inputs.apply` (`jules-archive-stale-true` vs `jules-archive-stale-false`) with `cancel-in-progress: false`; dry-run can run concurrently with an in-flight apply (intentional asymmetry — operational TOCTOU acceptable because dry-run does not mutate). See `.github/instructions/fleet-operations.instructions.md` for runbook. | `cd scripts/fleet && bun run archive-stale-sessions.ts --older-than-days N --repo current [--apply]` |
| `jules-persona-learning.yml` | `workflow_dispatch` only (no scheduled trigger — MVP report-only/manual proposal creation) | Jules persona PR learning loop (U1–U7 of the 2026-08-10 plan; U1–U6 implement collection and workflow wiring, and U7 is the ADR-037 governance lane). `collect` job (`contents: read` only) always runs: paginates Bolt/Sentinel PR evidence and builds a run-bound report artifact; the report always carries `session_verification: "none"` because no authoritative Jules session/source registry is wired up yet, so **`propose` mode is currently unavailable end-to-end by design** — `propose-cli.ts` refuses to proceed past artifact-binding validation whenever `session_verification` is not `"authoritative"` (see `schema/jules-pr-learning-contract.md`). `propose` job (`contents: write`, `pull-requests: write`, job-scoped only; also gated on the `jules-persona-learning-propose-approval` GitHub Environment — configure required reviewers in Settings → Environments) runs only when `mode=propose`: validates the same-run artifact's bindings (producer workflow, collector commit/run id, base SHA, expiry, digest, session_verification), re-verifies R6 eligibility for the operator-supplied evidence PR numbers, computes the candidate fingerprint server-side, deduplicates against live `.jules/*.md` memory and open/closed proposal markers, and opens exactly one human-reviewed PR on a `jules-memory/<persona>/<fp>` branch. Never merges/closes/redispatches its own PR — `fleet-merge.yml` and `fleet-dispatch.yml` explicitly exclude the `jules-memory/*` branch prefix. Uses the default `github.token` (never the `fleet-orchestrator` App token, per that action's own "PR-opening workflows must not invoke this" contract). Coordination across separate Actions runners uses a non-canceling `concurrency.group` plus GitHub-visible proposal-marker lookup and live base-tree/blob-SHA revalidation — not an ADR-005 local file lock, which cannot coordinate independent runners (see ADR-037). | `cd scripts/fleet && bun run pr-learning/collect-and-report-cli.ts <path>` (collect); `cd scripts/fleet && bun run pr-learning/propose-cli.ts` (propose) |
| `sweep-stale-bot-branches.yml` | Nightly schedule (cron `0 2 * * *`, 02:00 UTC), `workflow_dispatch` with boolean `dry_run` input (default `true`) | Scans bot-authored branches matching `^(jules/|google-labs-jules/|copilot/)` for branches older than 14 days with no open PR. Ships in **dry-run mode** (observation window, issue #346); real deletes gated by `sweep-real-delete-approval` GitHub Environment (created in issue #351 after 14 clean dry-run nights). `dry_run=false` workflow_dispatch attempts fail closed until that environment exists. `permissions: contents: read` only during observation window. Post-filter exclude blocklist (`main`, `fleet-state`, `gh-pages`) applies as defense-in-depth. | `python -m scripts.maintenance.sweep_stale_bot_branches --stale-days 14 --dry-run` |
...
- **Phase 2a `fleet-orchestrator` App token** (Layer 6, Issue #310, ADR-036): Phase 2a (`fleet-dispatch.yml`) prefers a `fleet-orchestrator` GitHub App installation token for `gh pr merge --auto` when both `FLEET_APP_ID` and `FLEET_APP_PRIVATE_KEY` repository secrets exist and token creation succeeds. The `fleet-orchestrator` App is a dedicated write-capable identity distinct from the narrow read-only `kb-source-monitor` App (`GH_APP_ID`/`GH_APP_PRIVATE_KEY`). Operator setup: provision the App via the one-click manifest in [`raw/inbox/cloneable-template.md` § GitHub App for Fleet Orchestration](../raw/inbox/cloneable-template.md#github-app-for-fleet-orchestration-issue-310-adr-036), install it on `wryenmeek/knowledgebase`, and add the two repository secrets under Settings → Secrets and variables → Actions. Until then, Phase 2a intentionally falls back to `GITHUB_TOKEN` and emits a warning annotation; the resulting squash-merge push to `main` does **not** fire push-triggered workflows (documented GitHub Actions behavior). Symptom: planning PR shows merged by `app/github-actions`, no Phase 2b run, no per-task agent sessions appear, `gh api repos/<owner>/<repo>/commits/<merge_sha>/check-runs` returns empty. Recovery: `gh workflow run fleet-dispatch-after-merge.yml --ref main` — the operator escape hatch re-resolves the planning artifact via fleet-state's `pending_date` and runs the same detect/clear/dispatch path as a normal push trigger. Issue #310 remains open until a real planning PR auto-merge proves Phase 2b fires automatically with the App token.

  - **App permission requirements:** The `fleet-orchestrator` App must grant `Contents: Read & write`, `Pull requests: Read & write`, `Issues: Read & write`, and `Metadata: Read` as **installation** permissions. The one-click manifest in `raw/inbox/cloneable-template.md` pre-fills these correctly; if you change permissions later, the operator must **re-accept** the installation grant at `Settings → Developer settings → GitHub Apps → fleet-orchestrator → Install App → wryenmeek → Configure` (GitHub requires explicit user approval after permission changes — silent grants are blocked by design). Do **not** widen the `kb-source-monitor` App — that App stays narrow read-only for CI-5 source ingestion.
  - **Diagnostic checklist when warning appears:**
    1. CI log search for `Failed to create token` or HTTP `422`/`401`/`404` in the `Create GitHub App token for auto-merge` step output.
    2. `422 "The permissions requested are not granted to this installation"` → `fleet-orchestrator` App's installation grant is missing one of `contents:write`/`pull_requests:write`/`issues:write`; edit at App Permissions & events, then re-accept installation.
    3. `401 "JSON web token could not be decoded"` → `FLEET_APP_PRIVATE_KEY` PEM is malformed (re-export, ensure full `-----BEGIN/END-----` markers, single secret value).
    4. `404 "Not Found"` → `fleet-orchestrator` App is not installed on this repository (install or re-install from the App's public page).
    5. After fixing, force a Phase 2a re-run by closing+reopening (or merge-then-revert) a planning PR; verify the step log emits `✅ Issue #310: using fleet-orchestrator App installation token for planning PR auto-merge.` and the resulting merge commit fires a `push`-event run on `fleet-dispatch-after-merge.yml`.
- Local validation commands:
  - `cd scripts/fleet && bun test`
  - `cd scripts/fleet && bun build fleet-analyze.ts fleet-plan.ts fleet-dispatch.ts fleet-merge.ts fleet-submit-prs.ts --target bun --outdir dist`
