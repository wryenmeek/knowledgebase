...
| CI-2 read-only diagnostics | `.github/workflows/ci-2-analyst-diagnostics.yml` on `push`, `pull_request`, or `workflow_dispatch` | Read-only permissions (`actions/checks/contents/issues: read`) plus artifact upload only; no repo mutations. |
