...
- **Optional Phase 2a GitHub App token** (Layer 6, Issue #310): Phase 2a (`fleet-dispatch.yml`) now prefers a GitHub App installation token for `gh pr merge --auto` when both `GH_APP_ID` and `GH_APP_PRIVATE_KEY` repository secrets exist and token creation succeeds. Operator setup is still required: create or reuse a repo-installed GitHub App with `contents: write` and `pull-requests: write`, install it on `wryenmeek/knowledgebase`, and add those two repository secrets under Settings → Secrets and variables → Actions. Until then, Phase 2a intentionally falls back to `GITHUB_TOKEN` and emits a warning annotation; the resulting squash-merge push to `main` does **not** fire push-triggered workflows (documented GitHub Actions behavior). Symptom: planning PR shows merged by `app/github-actions`, no Phase 2b run, no per-task agent sessions appear, `gh api repos/<owner>/<repo>/commits/<merge_sha>/check-runs` returns empty. Recovery: `gh workflow run fleet-dispatch-after-merge.yml --ref main` — the operator escape hatch re-resolves the planning artifact via fleet-state's `pending_date` and runs the same detect/clear/dispatch path as a normal push trigger. Issue #310 remains open until a real planning PR auto-merge proves Phase 2b fires automatically with the App token.

  - **App permission requirements (HITL operator step):** The App backing `GH_APP_ID` must grant **both** `Contents: Read & write` and `Pull requests: Read & write` as **installation** permissions (not just App-level permissions). Two distinct UI surfaces are involved: **(a) edit** the App's permission set at `Settings → Developer settings → GitHub Apps → your-app → Permissions & events`, then **(b) verify and re-accept** the installation grant at `Settings → Developer settings → GitHub Apps → your-app → Install App → wryenmeek → Configure`. If either permission is missing, `actions/create-github-app-token` will return HTTP `422 "The permissions requested are not granted to this installation."` and Phase 2a will fall back to `GITHUB_TOKEN`. After granting new permissions in (a), the operator must **re-accept** the installation prompt in (b) (GitHub requires explicit user approval after permission changes — silent grants are blocked by design).
  - **Diagnostic checklist when warning appears:**
    ...
    2. `422` → permission grant gap (above).
    3. `401 "JSON web token could not be decoded"` → private-key PEM is malformed (re-export, ensure full `-----BEGIN/END-----` markers, single secret value).
    4. `404 "Not Found"` → App is not installed on this repository (install or re-install from the App's public page).
    5. After fixing, force a Phase 2a re-run by closing+reopening (or merge-then-revert) a planning PR; verify the step log emits `✅ Issue #310: using GitHub App installation token for planning PR auto-merge.` and the resulting merge commit fires a `push`-event run on `fleet-dispatch-after-merge.yml`.
