...
- **Tiered path filter with benign allowlist:** pushes may include benign non-inbox paths (`docs/**`, `tests/**`, `wiki/pages/**`, `wiki/sources/**`, `README.md`, `LICENSE*`) alongside `raw/inbox/**` without rejection. Paths outside both the inbox and the benign allowlist are treated as sensitive control-plane paths and rejected with `reject:path_filter:sensitive_control_plane_path:*`.
...
| **CI-3** (`.github/workflows/ci-3-pr-producer.yml`) | write-capable PR producer after trusted handoff/manual approval with split trust boundary: low-privilege `synthesis-curator` job (`contents: read`, `models: read`) runs `extract_entities.py` and uploads deterministic extraction bundle artifacts (`ci3-synthesis/extraction-bundles.tsv` + bundle JSON files), then write-capable `pr-producer` job (`contents: write`, `pull-requests: write`, no `models` scope) applies bundles via `synthesize_combined.py`, runs ingest/index/lint/persist checks, and opens/updates PR; synthesis extraction remains soft-fail (`::warning`, continue) so LLM outages do not block ingest PR flow | execute the local sequence in this runbook, commit only allowlisted paths (`wiki/**`, `wiki/index.md`, `wiki/log.md`, `raw/processed/**`, `raw/rejected/**`) plus the CI-3 exception for `raw/inbox/**` deletions, and open/update PR manually through normal approvals/checks. Manual dispatch runs additionally require protected-environment reviewer approval (`ci3-manual-approval`) and now fail closed with explicit reason codes when the dispatch commit touches sensitive control-plane paths (`reject:path_filter:sensitive_control_plane_path:*`, `reject:trusted_trigger_model:manual_dispatch_sensitive_paths_present`). Push-triggered CI-3 preflight also fails closed when changed-path discovery is unavailable (`prereq_missing:ghaw_readiness:push_changed_paths_unavailable`) or when a changed file is outside the push allowlist (`reject:path_filter:disallowed_push_path:*`). If synthesis fails locally: check extraction bundle JSON for `"soft_skipped": true` (all 3 LLM attempts failed) or `::warning` output from extraction/synthesis steps; synthesis failures do not prevent the PR from opening. |
...
| `copilot-setup-steps.yml` | `workflow_dispatch`, `push`/`pull_request` on `.github/workflows/copilot-setup-steps.yml` and `scripts/fleet/**` | Configures Copilot cloud agent environment with Python 3.12, Bun, and all project dependencies; enforces fleet Bun tests/build on workflow-triggered runs | Push workflow changes to default branch; runs automatically when Copilot cloud agent starts a session |
