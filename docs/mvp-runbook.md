...
| `copilot-setup-steps.yml` | `workflow_dispatch`, `push`/`pull_request` on `.github/workflows/copilot-setup-steps.yml` | Configures Copilot cloud agent environment with Python 3.12, Bun, and all project dependencies; enforces fleet Bun tests/build on workflow-triggered runs | Push workflow changes to default branch; runs automatically when Copilot cloud agent starts a session |
