...
| `jules-account-probe.yml` | `workflow_dispatch` only | Read-only Jules account health diagnostic: lists registered sources, active/inProgress session counts per source, and account totals. Attach JSON output artifact for operator review. Use to diagnose session-cap saturation (see #82). | `cd scripts/fleet && bun run jules-account-probe.ts` |
